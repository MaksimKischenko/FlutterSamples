class RouteHelper {
  ...
  static bool _isHashStrategy = false;
  ...
  static MapEntry<String, Map<String,String>> parsePath(
    ...
    if (isHashStrategy ?? _isHashStrategy) {
      ...
      return MapEntry(uri.path.substring(_urlAppend.length), uri.queryParameters); 
    }
  }
  ...
  static MapEntry<String, String> parsePathWithStringParams(
    ...
    if (isHashStrategy ?? _isHashStrategy) {
      ...
      _print(uri.path.substring(_urlAppend.length), uri.queryParameters);
      return MapEntry(uri.path.substring(_urlAppend.length), uri.query.isEmpty ? uri.path.substring(_urlAppend.length) : '${uri.path.substring(_urlAppend.length)}?${uri.query}');
    }
  }
